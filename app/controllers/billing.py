...
def pay_tests(data: dict = Body(...), db: Session = Depends(get_db)):
    ...
    return {"message": "Bill generated", "bill_id": bill.id,
            ...
            "total": total, "discount": discount, "net_total": net}
...
def get_patient_bills(patient_id: int, db: Session = Depends(get_db)):
    ...
    return {"bills": [
        {"bill_id": b.id, "date": fmt_dt(b.paid_at), "total": b.total_amount,
         "discount": b.discount, "net_total": b.net_total,
...
def get_invoice(bill_id: int, db: Session = Depends(get_db)):
    ...
    return {
        "bill": {"bill_id": bill.id, "date": fmt_dt(bill.paid_at), "total": bill.total_amount,
                 "discount": bill.discount, "net_total": bill.net_total,
...
def admin_get_patient_bills(patient_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return {"bills": [
        {"bill_id": b.id, "date": fmt_dt(b.paid_at), "total": b.total_amount,
         "discount": b.discount, "net_total": b.net_total,
         "payment_method": b.payment_method, "bill_type": b.bill_type or "counter"}
        for b in rows
    ]}
