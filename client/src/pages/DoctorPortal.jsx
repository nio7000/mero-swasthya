...
export default function DoctorPortal() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setReports((rr.data.reports || []).map((r, i) => ({
          id: r.id ?? i + 1,
          structured_data:  r.structured_data,
          medical_analysis: r.medical_analysis,
        })));
  ...
  const handleSelectPatient = (r) => {
    const name = (r.structured_data?.["Patient Name"] || "").replace(/^Mr\.?\s*/i, "").trim();
    setSearchTerm(name);
  ...
  const filteredReports = reports.filter(r => {
    ...
    if (!q) return false;
    const name    = (r.structured_data?.["Patient Name"] || "").toLowerCase();
    const pid     = (r.structured_data?.["Patient ID"]   || "").toLowerCase();
    const contact = (r.structured_data?.["Contact"]      || "").toLowerCase();
        ...
        <div className="panel">
          <div className="panel-top">
            ...
            <div className="search-wrap">
              ...
              />
              {showSuggestions && searchTerm && filteredReports.length > 0 && (
                <div className="drop">
                  {filteredReports.map((r, i) => {
                    const pname = (r.structured_data?.["Patient Name"] || "").replace(/^Mr\.?\s*/i, "").trim();
                    const pid   = r.structured_data?.["Patient ID"] || "";
                    const age   = r.structured_data?.["Age"] || "";
                    const sex   = r.structured_data?.["Sex"] || "";
                    return (
                      <div key={i} className="drop-item" onMouseDown={() => handleSelectPatient(r)}>
                        <span style={{fontWeight:600}}>{pname}</span>
                        <span style={{color:"var(--text3)",fontSize:13,marginLeft:10}}>{pid}{age?` · ${age}y`:""}{sex?` · ${sex}`:""}</span>
                      </div>
                    );
                  })}
                </div>
                    ...
                    <div key={i} className="rx-card">
                      <div className="rx-card-top">
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                          ...
                          {/* Follow-up button — only on most recent prescription */}
                          {i === 0 && (
                            <button
                              className={`btn-followup ${isAssigned ? "assigned" : ""}`}
                              onClick={() => handleFollowUp(selectedPatientId)}
                            >
                              {isAssigned ? "✓ Follow-up Assigned — Click to Cancel" : "+ Mark Follow-up"}
                            </button>
                          )}
                        </div>
                      </div>
